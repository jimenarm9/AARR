...
Traducido a las variables continuas que usa este proyecto:

```
degradación_residual = degradación × (1 − ei)
...
```
Impacto residual = impacto_tabla(valor, degradación_residual)
...
En vez de asignar 74 números a ojo, se usó una tabla fija de 9 valores (uno por tipo), **calibrada** — no copiada literalmente — con los porcentajes reales del **CIS Community Defense Model v2.0** (Center for Internet Security), que cuantifica con datos de MITRE ATT&CK y estadísticas de amenazas reales cuánto protege cada tipo de control frente a los 5 ataques más comunes (Ransomware, Malware, Ataques web, Abuso de privilegios, Intrusiones dirigidas). Es un catálogo distinto al ENS (CIS Controls v8, no Anexo II), así que el cruce sigue siendo criterio del proyecto — pero informado por datos reales, no inventado desde cero.
...
> **Corrección (09/09/2026):** los dos valores de este ejemplo estaban mal en la versión anterior del documento (decía Potencial=7,70 / Actual=2,43) — eran un cálculo manual con error aritmético, nunca reejecutado contra el código real antes de publicarse. Los valores de abajo están verificados ejecutando `calculo.py` directamente, y coinciden con el segundo ejemplo de esta misma sección (SRV-01), que sí estaba bien desde el principio.
...
Riesgo Actual:    3,98   (eficacia combinada: ei=85%, ep=40%)
```

Y verificación de propagación: activo INF-01 (sin salvaguardas propias) heredando de SRV-01 (con salvaguardas):

```
SRV-01 Directa   — Potencial: 6,54  →  Actual: 3,98
INF-01 Dependencia — Potencial: 5,00  →  Actual: 3,21   (protegido sin tener nada asignado)
```
...
Activo "011 · SO de servidores", amenaza de categoría "Explotación de vulnerabilidades / Malware", con la salvaguarda `[op.exp.6] Protección frente a código dañino` asignada:

```
Madurez L5: Riesgo Potencial 6,87  →  Riesgo Actual 6,10
Madurez L2: Riesgo Actual sube respecto al valor con L5 (menos eficacia efectiva) — confirmado en la app
```
